...
def create_room(request):
    room = Room.objects.create(
        name=request.data.get("name"), room_type=request.data.get("room_type")
    )

    emit_event("ROOM_CREATED", room, request.user)

    return Response({"room_id": room.id, "room_type": room.room_type}, status=201)
...
def join_room(request, room_id):
    ...
    # already joined?
    if RoomMember.objects.filter(room=room, user=user, left_at__isnull=True).exists():
        return Response(status=204)

    active_count = RoomMember.objects.filter(room=room, left_at__isnull=True).count()
...
def leave_room(request, room_id):
    membership = RoomMember.objects.filter(
        room_id=room_id, user=request.user, left_at__isnull=True
    ).first()
