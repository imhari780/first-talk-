...
from .serializers import DialogueMessageInputSerializer, DialogueSessionSerializer
...
def start_dialogue(request):
    ...
    if DialogueSession.objects.filter(
        user=request.user, ended_at__isnull=True
    ).exists():
        return Response(
            {"error": "Active session already exists"}, status=status.HTTP_409_CONFLICT
        )
...
def send_message(request, session_id):
    ...
    try:
        session = DialogueSession.objects.get(
            session_id=session_id, user=request.user, ended_at__isnull=True
        )
    except DialogueSession.DoesNotExist:
        return Response(
            {"error": "Invalid or ended session"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    DialogueMessage.objects.create(
        session=session, sender_type="system", content=system_reply
    )

    # ❌ No history returned
    return Response({"content": system_reply}, status=status.HTTP_200_OK)
